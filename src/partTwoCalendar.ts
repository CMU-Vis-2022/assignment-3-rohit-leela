...
export function Calendar(
  ...
  const years = d3.groups(I, (i) => {
    const date = moment(X[i], "DD-MM-YYYY").toDate();
  ...
  const cell = year
    ...
    .data(
      weekday === "weekday"
        ? ([, I]) => {
          return I.filter((i) => {
            var day = moment(X[i], "DD-MM-YYYY").toDate().getUTCDay();
    ...
    .attr(
      "x",
      (i) => timeWeek.count(
        d3.utcYear(moment(X[i], "DD-MM-YYYY").toDate()),
        moment(X[i], "DD-MM-YYYY").toDate()
      ) *
        cellSize +
        0.5
    )
    .attr(
      "y",
      (i) => countDay(moment(X[i], "DD-MM-YYYY").toDate().getUTCDay()) * cellSize +
        0.5
    )
  ...
  const month = year
    ...
    .selectAll("g")
    .data(([, I]) => d3.utcMonths(
      d3.utcMonth(moment(X[I[0]], "DD-MM-YYYY").toDate()),
      d3.utcMonth(moment(X[I[I.length - 1]], "DD-MM-YYYY").toDate())
    )
    )
    .join("g");
